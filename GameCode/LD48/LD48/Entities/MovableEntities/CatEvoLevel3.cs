...
namespace LD48
{
    public class CatEvoLevel3 : Cat
    {
        ...
        {
            Texture = SharedContext.Content.Load<Texture2D>("Images/catEvoLevel3");

            CollisionBox = new Rectangle(14, 20, 94, 70);

            LineOfSite = new Rectangle(-110, -110, 350, 350);
        ...
        public override void Draw(GameTime gameTime, SpriteBatch spriteBatch)
        {
            ...
            {
                spriteBatch.Draw(Texture, new Rectangle((int)WorldPosition.X, (int)WorldPosition.Y, 60, 42), null, Color.White, 0, Vector2.Zero,
                       !_isFacingRight ? SpriteEffects.FlipHorizontally : SpriteEffects.None, layerDepth);
            }
            else
            {
                spriteBatch.Draw(Texture, new Rectangle((int)WorldPosition.X, (int)WorldPosition.Y, 120, 85), null, Color.White, 0, Vector2.Zero,
                       !_isFacingRight ? SpriteEffects.FlipHorizontally : SpriteEffects.None, layerDepth);
            }

            if (GestationState == GestationStateEnum.Breeding)
            {
                spriteBatch.Draw(CensoredTexture, new Rectangle((int)WorldPosition.X + 20, (int)WorldPosition.Y + 5, 110, 35), null, Color.White, 0, Vector2.Zero,
        ...
        protected override void HandleCollision(ICollidable collider)
        {
            ...
            if (collider is CatEvoLevel3 || collider is CatEvoLevel2)
            {
